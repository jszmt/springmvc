package com.wugz.mvc.mvcframework.servlet;


import com.wugz.mvc.mvcframework.annotations.*;
import com.wugz.mvc.mvcframework.pojo.Handler;
...
public class DispatcherServlet extends HttpServlet {
    ...
    /** 权限校验key */
    private static final String SECURITY_KEY = "username";
...
//        method.invoke() //
        ...
        List<String> securitys = handler.getSecuritys();
        /**  如果权限验证不为空 则进行验证*/
        if(!securitys.isEmpty()){
            /** 从参数中获取当前用户名 */
            String [] securityParam = parameterMap.get(SECURITY_KEY);
            /** 如果参数中没有带用户名 或 用户名不符合权限要求 则返回 403 只验证 */
            if(securityParam == null || securitys.isEmpty() || !securitys.contains(StringUtils.join(securityParam, ","))){
                /** 返回没有权限 */
